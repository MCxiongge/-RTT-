...
#include <stdint.h>
#include <stdbool.h>
typedef struct rx_conf_s {
    uint32_t    freq_hz;        /*!> central frequency of the IF chain */
    uint32_t     modulation;     /*!> modulation used by the packet */
    uint32_t     bandwidth;      /*!> modulation bandwidth (LoRa only) */
    uint32_t    datarate;       /*!> RX datarate of the packet (SF for LoRa) */
    uint32_t     coderate;       /*!> error-correcting code of the packet (LoRa only) */
}rx_conf_t;
/* Private typedef -----------------------------------------------------------*/
/*!

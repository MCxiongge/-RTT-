...
static rt_uint8_t rt_lora_radio_thread_stack[1024];
#endif
#ifdef LORA_RADIO_DRIVER_USING_RXDONE_TIMESTAMP
/*!
 * \brief rx timestamp
 */
volatile uint32_t radio_rxdone_timestamp = 0;
#endif
...
/* ST_WORKAROUND_END */
#ifdef LORA_RADIO_DRIVER_USING_RXDONE_TIMESTAMP
uint32_t RadioGetRxDoneTimestamp(void);
#endif
...
static uint32_t radio_dio_irq_timestamp_us = 0;
static void RadioOnDioIrq( RadioIrqMasks_t radioIrq )
{
    SubgRf.RadioIrq = radioIrq;

    //RADIO_IRQ_PROCESS();
    //rt_kprintf("RadioOnDioIrq\r\n");
    #ifdef LORA_RADIO_DRIVER_USING_RTOS_RT_THREAD
#ifdef LORA_RADIO_DRIVER_USING_RXDONE_TIMESTAMP
    if( SUBGRF_GetOperatingMode( ) == MODE_RX )
    {
#if defined PKG_USING_MULTI_RTIMER
            radio_dio_irq_timestamp_us = (rtimer_get_current_time()*1000);
            LOG_D("radio_rxdone_timestamp = %u",radio_rxdone_timestamp);
            //radio_dio_irq_timestamp_us = TimerGetCurrent_us(); // us
#else
            radio_dio_irq_timestamp_us = TimerGetCurrentTime() / RT_TICK_PER_SECOND * 1000000; // us 
#endif
    }  
#endif
...
//        }
    ...
    case IRQ_RX_DONE:
        ...
        /* ST_WORKAROUND_END */
        #ifdef LORA_RADIO_DRIVER_USING_RXDONE_TIMESTAMP
                radio_rxdone_timestamp = radio_dio_irq_timestamp_us;
        #endif
        TimerStop( &RxTimeoutTimer );
